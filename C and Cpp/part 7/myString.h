...
public:
	...
	void checkSubstring(const char *str) 
	{
		int length = 0;	
		int counter = 0;
		int k = 0;
		...
		for(int i = 0; i < m_length; ++i)
		{
			if(str[k] == m_data[i])
			{
				...
				if(counter == length)
				{
					std::cout << "Yes the Substring exisits\n";
		...
		std::cout << "No substring is absent.\n";
	}
...
bool operator==(const myString &s1,const myString &s2) // compare 2 strings
{
